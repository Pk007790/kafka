 ...
 */
public class MultiTopicStateStoreDriver {

    public static final String CONSUMER_GROUP_ID = "GKTmultiTopiccons";
    ...
    public static Properties getConsumerProps() {
        ...
        properties.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, MultiTopicStateStore.BOOTSTRAP_SERVER);
    ...
    public static void produceInput(){
        ...
        producerProps.put(ProducerConfig.BOOTSTRAP_SERVERS_CONFIG, MultiTopicStateStore.BOOTSTRAP_SERVER);
        ...
        IntStream.range(0,MAX_RECORDS)
                .forEach(record -> {
                    String message = msg[random.nextInt(msg.length)];
                    producer.send(new ProducerRecord<String, String>
                            (MultiTopicStateStore.INPUT_TOPICA, null, message));  //0,1,2..4

                    producer.send(new ProducerRecord<String, String>
                            (MultiTopicStateStore.INPUT_TOPICB, null, message)); //0,1,2..4

                    producer.send(new ProducerRecord<String, String>
                            (MultiTopicStateStore.INPUT_TOPICC,null, message)); //0,1,2..4
                });
        producer.flush();
    }
    public static void consumerOutput() throws InterruptedException {
        ...
        consumer.subscribe(Collections.singleton(MultiTopicStateStore.OUTPUT_TOPIC));
        while (true) {
            Thread.sleep(5_000);
